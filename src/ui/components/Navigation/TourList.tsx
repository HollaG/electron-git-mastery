...
export const TourList = () => {
  ...
  return (
    <Stack>
      ...
      {tourList
        ? Object.values(tourList).map((tour, index) => (
            <TourItem key={index} tour={tour} index={index} />
          ))
        : "No data"}
    </Stack>
  );
...
const TourItem = ({ tour, index }: { tour: Tour; index: number }) => {
  ...
  return (
    <Flex direction={"column"} key={tour.title}>
      <Tooltip
        label={tour.title}
      ...
      <Collapse in={opened} p="8px" w={"100%"}>
        <NavigationButton
          title="Tour Home"
          onClick={() => navigate(buildTourHomeUrl(tour))}
        />
        {Object.values(tour.lessons).map((lesson) =>
          buildLesson(lesson, navigate),
        )}
      </Collapse>
    </Flex>
  );
...
const buildLesson = (lesson: Lesson, navigate: (url: string) => void) => {
  return (
    <NavigationButton
      title={lesson.title}
      onClick={() => navigate(buildLessonUrl(lesson))}
    />
  );
};
